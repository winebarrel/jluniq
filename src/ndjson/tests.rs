...
fn test_uniq() {
    ...
    uniq(reader, "id", fout, Opts { group: false }).unwrap();
...
fn test_uniq_with_grouping() {
    ...
    uniq(reader, "id", fout, Opts { group: true }).unwrap();

    assert_eq!(
        indoc! {r#"
            [1,{"id":1,"sub_id":11,"val":"abc"}]
            [2,{"id":2,"sub_id":22,"val":"ghi"}]
...
fn test_join_without_key() {
    ...
    let r = uniq(reader, "noid", fout, Opts { group: false });
